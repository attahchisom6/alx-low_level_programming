...
ADDING LARGE NUMBERS STORED IN TWO STRINGS
...
in this project we are with the assumptoon that we are dealing with a large number of unsigned integers and will primarily apply elementary mathematics to our sum
e.g		57
		+5
		__
		62
...
2. if the sum of this digits is lessthan 10, then just print their sum, however, if thier sum is greater than 10, as in seen in our example above 5 + 7 = 12, then we will write down the last digit 2, carry the first digit 1, and add it to the next number 5.
...
1. p and k are the running counter to the elements of the array n1[] and n2[] respectively.
2. digits is the running counter to the buffer r[]
3. value1 and value2 of the elements of the arr n1[] and n2[] we are chrrently adding starting from end.
...
char *infinite_add(char *n1, char *n2, char *r, int size_r)
{
	...
	while (p >= 0 || k >= 0 || carry_over == 1)
	{
		if (p < 0)
			...
			value1 = n1[p] - '0'; /*convert character to integer numbers*/
		if (k < 0)
			return (0);
		else
			value2 = n2[k] - '0';
